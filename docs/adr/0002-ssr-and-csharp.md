...
> 게스트 업로드에 관한 근거는 당시의 범위를 설명한다. 해당 기능은 [ADR 0006](0006-remove-guest-upload.md)으로 제거했으며 C# + Razor SSR 결정은 유지한다.
...
- JS 없으면 업로드 폼 POST로 동작하고 JS 있으면 전송 진행률이 붙는 점진적 향상이다. htmx 4 전환에 따른 전송 구현은 아래 후속 갱신에 기록한다
- SSR 쪽 개발 루프도 공짜는 아니다. devel의 `run_dev.sh`는 시작할 때마다 CSS를 새로 빌드했고 HMR이 없었다

### 후속 갱신 — 2026-09-25

htmx 4.0.0으로 전환하면서 기존 `htmx:xhr:progress` 이벤트가 제거됐다. 기본 Fetch 전송은 업로드 진행률을 제공하지 않으므로, 전송률 퍼센트를 유지하기 위해 업로드 폼에 한해 `ctx.fetch`를 XHR 기반 전송으로 재정의한다. 공식 파일 업로드 문서는 진행률이 필요하면 별도 업로드 도구를 검토하도록 안내하고, 확장 작성 가이드는 `ctx.fetch` 재정의를 지원한다. 이 확장 지점을 이용한 커스텀 전송의 유지보수는 애플리케이션이 맡는다. 상세 구현과 공식 근거는 [DESIGN.md](../DESIGN.md#htmx-4와-업로드-진행률)에 정리한다.

htmx의 응답 스트리밍만 도입해도 브라우저의 업로드 전송률을 얻을 수 있는 것은 아니다. 서버 수신·저장 진행률을 측정하고 전달하는 별도 기능은 이번 전환에 포함하지 않는다. C# + Razor SSR과 일반 폼 POST를 유지하는 결정은 그대로다.
