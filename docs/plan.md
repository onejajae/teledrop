...
> 2026-09-21: [ADR 0006](adr/0006-remove-guest-upload.md)에 따라 게스트 업로드 티켓을 제거했다. 아래 스테이지 6·12와 티켓 관련 검증은 구현 당시 이력이며 현재 범위가 아니다. 기존 DB 전환 마이그레이션은 제공하지 않는다.

## 모든 스테이지 지시문에 공통으로 들어가는 계약

- 시작 전에 `docs/CONTEXT.md`(용어), `docs/DESIGN.md` 해당 절, 관련 ADR을 읽고 따른다
- 코드 이름에 `docs/CONTEXT.md`의 용어를 그대로 쓴다 (Drop, Slug — Content/Post/Key가 아니다)
...
- [ ] README 재작성 — 태그라인(`powered by REST API` 삭제)과 설치 문서 전체(환경변수·소유자 세션·API 키 업로드)
