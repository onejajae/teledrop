...
둘을 함께 전달한다. "이 링크 들어가서 이 코드 쳐." 둘 다 **혼동 문자(`0 o`, `1 l i`)를 뺀 31자 영숫자 알파벳**(`abcdefghjkmnpqrstuvwxyz23456789`)에서 뽑고, 입력받을 때 하이픈과 대소문자는 무시한다.

> 처음 기획에서 "base32"라 적었으나 이는 부정확했다. 소문자+숫자 36자에서 혼동 문자 5개를 빼면 31자라 32자 base32가 성립하지 않는다. 실제 요구는 "전화로 불러줄 수 있고, 받아 적을 수 있고, 추측 불가"였고 31자 알파벳이 그것을 충족한다. 엔트로피는 경로 4자 ≈ 2^19.8, 코드 8자 ≈ 2^39.6으로 32자 기준(2^20/2^40)과 사실상 동일하다.
