...
class TestAppContainer:
    ...
    def test_ensure_app_container_builds_when_settings_are_provided(self):
        app = FastAPI()
        settings = _FakeSettings()
        fake_container = MagicMock(spec=AppContainer)
        with patch('app.bootstrap.container.build_app_container', return_value=fake_container) as build_mock:
            result = ensure_app_container(app, settings=settings)
        assert result is fake_container
        assert app.state.container is fake_container
        build_mock.assert_called_once_with(settings)

    def test_ensure_app_container_requires_explicit_settings(self):
        app = FastAPI()

        with pytest.raises(RuntimeError, match='App container is not attached'):
            ensure_app_container(app)
...
class TestBareFastApiFallback:
    ...
    def test_bare_fastapi_route_reads_settings_from_attached_container(self):
        app = FastAPI()
        settings = _FakeSettings()
        container = MagicMock(spec=AppContainer)
        container.settings = settings
