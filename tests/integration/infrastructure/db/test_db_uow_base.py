...
class TestBaseSQLModelUnitOfWork:
    def setup_method(self):
        ...
        self.bound_repository = MagicMock()
    ...
    async def test_aexit_without_exception_closes_session(self):
        ...
        self.bound_repository.deactivate.assert_called_once_with()
    ...
    def test_repository_raises_without_active_session(self):
        with pytest.raises(RuntimeError, match="inactive session"):
            _ = self.uow.repository
    ...
    async def test_repository_raises_after_exit(self):
        await self.uow.__aenter__()
        await self.uow.__aexit__(None, None, None)
