...
class TestDetailPanelPresenter:
    ...
    async def test_first_password_prompt_does_not_show_error_message(self):
        context = await detail_panel_context(request=self.request, auth_data=self.auth_data, csrf_service=self.csrf_service, drop_use_cases=self.drop_use_cases, settings=self.settings, selected_key='locked', selected_password=None)
    ...
    async def test_wrong_password_shows_invalid_password_message(self):
        context = await detail_panel_context(request=self.request, auth_data=self.auth_data, csrf_service=self.csrf_service, drop_use_cases=self.drop_use_cases, settings=self.settings, selected_key='locked', selected_password='bad-password')
    ...
    async def test_access_denied_sets_forbidden_error_code(self):
        context = await detail_panel_context(request=self.request, auth_data=self.auth_data, csrf_service=self.csrf_service, drop_use_cases=_FakeDropUseCasesForbidden(), settings=self.settings, selected_key='locked')
    ...
    async def test_not_found_sets_not_found_error_code(self):
        context = await detail_panel_context(request=self.request, auth_data=self.auth_data, csrf_service=self.csrf_service, drop_use_cases=_FakeDropUseCasesNotFound(), settings=self.settings, selected_key='missing')
