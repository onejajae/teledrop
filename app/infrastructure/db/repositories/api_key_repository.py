...
class SQLModelApiKeyReadRepository:
    ...
    async def list_for_owner(self, owner_user_id: str) -> list[AuthApiKeyRecord]:
        return await anyio.to_thread.run_sync(
            self._list_for_owner_with_new_session,
            owner_user_id,
        )

    def _list_for_owner_with_new_session(self, owner_user_id: str) -> list[AuthApiKeyRecord]:
        def operation(session: Session) -> list[AuthApiKeyRecord]:
            query = (
                select(AuthApiKey)
                .where(AuthApiKey.owner_user_id == owner_user_id)
                .order_by(AuthApiKey.created_at.desc())
            )
